...
def bfs(max_r, max_c):
    ...
    q.append((max_r, max_c, max_h, -1, l, gongsa)) 
    ...
    while q:
        rdx, cdx, hdx, bf_d , l, gongsa = q.pop()
                ...
                if h>=hdx:
                    if gongsa:# 공사 했으면 
                        ...
                        for kk in range(1, K+1):
                            if h-kk < hdx:
                                q.append((r,c,h-kk,d,l+1, True))
                                found = True
                                break
                    #print('공사하면서 깍자', (r,c,h-1,d,l+1, k-1))
        
        if found is False:
            visited_set.discard((rdx,cdx))
